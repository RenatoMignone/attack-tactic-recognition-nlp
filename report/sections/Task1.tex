...
\paragraph{Labels exploration} \Cref{fig:task1_tags_distribution} illustrates the distribution of tags in both the training and test sets. The datasets comprise seven distinct tags: \emph{Defense Evasion}, \emph{Discovery}, \emph{Execution}, \emph{Impact}, \emph{Not Malicious Yet}, \emph{Other}, and \emph{Persistence}. Generally, the number of Bash words assigned to a tag is higher in the training set than in the test set. In fact, the training set contains 11475 Bash words distributed among 251 sessions, while the test set only 6197 (108 sessions). Both sets exhibit a similar distribution profile: \emph{Discovery} ($\approx\SI{50}{\percent}$) is the most prevalent tag, followed by \emph{Execution} ($\approx\SI{25}{\percent}$), and \emph{Persistence} ($\approx\SI{10}{\percent}$). The remaining four tags are significantly less frequent, each associated with less than \SI{4}{\percent} of Bash words.

\paragraph{The \texttt{echo} command}\label{sec:task1_echo_analysis} The \texttt{echo} command has been assigned to 6 different tags: \emph{Persistence} (104 times), \emph{Execution} (39 times), \emph{Discovery} (31 times), \emph{Not Malicious Yet} (8 times), \emph{Impact} (6 times), and \emph{Other} (4 times). We will now analyze two examples: one session in which \texttt{echo} is labelled as \emph{Persistence} and one as \emph{Execution}.

\textbf{(\texttt{echo}, Persistence):} \texttt{[\dots]; echo ``root:HGbB4i9gUXMh'' | chpasswd | bash ; [\dots]}. The \texttt{echo} command outputs the string \texttt{root:HGbB4i9gUXMh}, which is piped to \texttt{chpasswd}, that updates user passwords from standard input. By changing the \texttt{root} user's password to a known value, the attacker gains persistent administrative access to the compromised system.

\textbf{(\texttt{echo}, Execution):} \texttt{[\dots]; echo
``base64\_payload'' | base64 -{}-{}decode | bash ;}. The \texttt{echo} command outputs a base64 payload, that is then decoded and executed. Hence, this sequence executes arbitrary code, probably a malware binary.
